...
#region Horizontal Move/Collide
	x += _velocity_x;
...
#region Vertical Move/Collide
	y += _velocity_y;
	ds_list_clear(_list)
	var _collisions = instance_place_list(x, y + sign(_velocity_y), __collider, _list, false);
	...
	if(_collisions)
	{
		...
		else if (_velocity_y < 0)
		{
			for(var _i = 0; _i < _collisions; _i++;)
			{
				_resolved_y = max(_resolved_x, _list[| _i].bbox_bottom + y - bbox_top);
			}
		}
		y = _resolved_y;
	}
